...
if filenum(fi)<177 && filenum(fi)>155
    load([maskdir 'PIVlab_mask1_c0157-c0162.mat'])
elseif filenum(fi)<156
    load([maskdir 'PIVlab_mask1_c0149-c0155.mat'])
elseif filenum(fi)>176 
    load([maskdir 'PIVlab_mask1_c0177-c0182.mat'])
end
...
%     pixeltorealdimension=0.10955/835;%c0157-c0172
elseif filenum(fi)<156
    load(['C:\Users\jeffr\Dropbox (Sydney Uni)\projects\unsaturated\Roration drum experiment\Dataanalysis\pivlab_masks\' ...
...
elseif filenum(fi)>176
    load(['C:\Users\jeffr\Dropbox (Sydney Uni)\projects\unsaturated\Roration drum experiment\Dataanalysis\pivlab_masks\' ...
    'calibration_coe_c0177-c0182.mat']);
end
...
framesets=[1:10:1001]';
processquant=size(framesets,1);
displayprogress=[0:processquant/20:processquant]'/processquant;
for i=1:processquant
    progress=i/processquant*100;
    if ismember(progress,displayprogress)
        disp(['C0' num2str(filenum(fi,1)) 'Ongoing_' num2str(progress) '%']);
%     else
%         continue
    end
    disp(iminfo(framesets(i,1)).name);
    image=imread([filedir iminfo(framesets(i,1)).name]);
    % % 1. run Exp_strainrate_pivlab
    % % 2. load the image manuely e.g. C0083_00100
    [BW,maskedImage] = Exp_segmentImage_fun_grey_c0181(image);
    xtmp=[1:1920];
    ytmp=flip([1:1080]');
