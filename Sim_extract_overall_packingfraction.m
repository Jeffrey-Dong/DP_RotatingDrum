...
% modelnum=[141:150]';
% modelnum=[119,134,121:123,124,126:128,139,141:143]';
modelnum=[120,125,151,140,152]';
% modelnum=[123]';
%%
...
%         tic
    maxaccu_step=1;
    mid_maxaccu_step=ceil(maxaccu_step/2);
        for esri=1
    %         :maxaccu_step %each sampling range
            revolution(esri,1)=sample_range(samplei)*DEMstepinterval*DEMtimestep*omega/360;% for model 25-30 sample_range-24
            if modelnum(modeli,1)<118
                [atomnum,~,~]=xlsread( ...
                [Foldersdir_atominfo '\' txt_read_atominfo(sample_range(samplei)-mid_maxaccu_step+esri).name],1);
                idest=atomnum(:,1);
                x=atomnum(:,18);
                y=atomnum(:,19);
                z=atomnum(:,20);
                ...
                [idest,~,x,y,z,~,~,~,vx,vy,vz,~,~,~,~,~,~,radius_est,VliqonVatom]=textread( ...
                    txt_read_atominfo(sample_range(samplei)-mid_maxaccu_step+esri).name,'%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f','headerlines',9);
                vel=sqrt(vx.^2+vy.^2+vz.^2);
            end
    ...
    %         atom_vel_eachstep=cell2mat(velocity(sample_range(samplei+esri-1),1));
            ...
            framenum_record=[framenum_record;ones(Nptotal,1)*modeli];
...
tic
        ...
        framenum_record_cell=zeros(size(xgridpf,1)-1);
...
%                 veltincel(xgi,zgi)=mean(veltotalscaler(intersect(xrange,zrange),1));
                framenum_record_cell(xgi,zgi)=size(unique(framenum_record(xrange(ismember(xrange,zrange)))),1);
            end
        end

%         phi_incell=V_p.*Nincell/(abs(xgridpf(2)-xgridpf(1))*abs(zgridpf(2)-zgridpf(1))*container_d*samplenumber);%universal time average
          phi_incell=V_p*Nincell./(abs(xgridpf(2)-xgridpf(1))*abs(zgridpf(2)-zgridpf(1))*container_d*framenum_record_cell);%individual time frame average
...
cd(datafile);
datafilename=['model' num2str(modelnum(modeli,1)) '_overall_packingfra'];
save(datafilename,'Xpf');
save(datafilename,'Zpf','-append');
%    save(datafilename,'DAORt_fitraw','-append');
save(datafilename,'phi_incell','-append');
clear Xpf Zpf phi_incell tmpx tmpz
    %% ===========save results=============
%    cd(datafile);
