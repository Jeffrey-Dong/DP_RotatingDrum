...
%% plot velocity contour and locate the boundary
max_v=max(sqrt(Vxavereal.^2+Vyavereal.^2),[],'all');
vel=sqrt(Vxavereal.^2+Vyavereal.^2);
figure(5);[c3,h3]=contourf(xreal,yreal,vel,200);
set(h3, 'edgecolor','none');
axis equal;
colorbar;
vel(vel<=(omega/180*pi*0.1))=0;
[vel_bound_tmp]=contours(xreal,yreal,vel,[max_v*0.01 max_v*0.01]);
vel_bound_tmp=vel_bound_tmp';
% strr_bound_tmp(strr_bound_tmp(:,1)>=1,:)=[];%remove index array
vel_circle_index=find(vel_bound_tmp(:,2)==max(vel_bound_tmp(:,2)));
vel_bound_tmp1=vel_bound_tmp(vel_circle_index+1:vel_circle_index+max(vel_bound_tmp(:,2))-1,:);
% %-----------plot strain rate bound---------------------
figure(5);hold on;
plot(vel_bound_tmp1(2:end,1),vel_bound_tmp1(2:end,2),'r.');
xlim([0.08 0.28]);
...
%% method 2 extract shear region lower boundary based on strain rate: clusterdata function
cali=0.236/1295.64;
pixeltorealdimension=cali/0.96;
...
%% method 2 extract shear region lower boundary based on velocity: clusterdata function
cali=0.236/1295.64;
pixeltorealdimension=cali/0.96;
dp=0.002;%particle size
xws=2*dp;%x window size
steps=1*dp;%moving step size
[vel_botbound]=strainrate_bottombound(vel_bound_tmp1,xws,steps);
boundary_in_pixel_vel=[surface_x,surface_y;vel_botbound]/pixeltorealdimension;
boundary_in_pixel_vel(:,2)=(boundary_in_pixel_vel(:,2)-1080)*-1;
% datafilename=[char(expnum) '_vel_boundary_omegaRthresh.mat'];
% save(datafilename,'boundary_in_pixel_vel');

%% fitting and find the velocity depth
marker_color_type='ko';
dpfit=0.002;%particle size
xwsfit=10*dpfit;%x window size
stepsfit=1*dpfit;%moving step size
[vel_depth,vel_depth_normbydp,vel_maxdepth_norm,vel_swfit]=strr_depth_fun(vel_botbound,surface_x,surface_y,dp,xwsfit,stepsfit);
datafilename=([char(expnum) '_velocity_depth.mat']);
% save(datafilename,'vel_depth_normbydp');
% save(datafilename,'vel_maxdepth_norm','-append');
% save(datafilename,'vel_swfit','-append');


%%
strain_rate_original=strain(x,y,u_original,v_original);
%%
