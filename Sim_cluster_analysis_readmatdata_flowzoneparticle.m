...
% modelnum=[103:111]';
% modelnum=[113:114]';
modelnum=[121:123,124,126:128,134]';
% modelnum=[134]';
%%
...
%     tic
    ...
    id_centroids_radius_vxyz=[idest,x,y,z,radius_est,vx,vy,vz];
...
% %         tic
%         pl=[];
%         parfor pli=1:size(centroids_est,1)
%             if vxyz_est(pli,1)<0 && vxyz_est(pli,3)<0
%             xbound=[centroids_est(pli,1)-1.5*d_p centroids_est(pli,1)+1.5*d_p];
%             ybound=[centroids_est(pli,2)-1.5*d_p centroids_est(pli,2)+1.5*d_p];
%             zbound=[centroids_est(pli,3)-1.5*d_p centroids_est(pli,3)+1.5*d_p];
%             id_inthisloop=id_est(pli+1:end,1);
%             index_x1=id_inthisloop(centroids_est(pli+1:end,1)>xbound(1));
%             index_x2=id_inthisloop(centroids_est(pli+1:end,1)<xbound(2));
%             index_y1=id_inthisloop(centroids_est(pli+1:end,2)>ybound(1));
%             index_y2=id_inthisloop(centroids_est(pli+1:end,2)<ybound(2));
%             index_z1=id_inthisloop(centroids_est(pli+1:end,3)>zbound(1));
%             index_z2=id_inthisloop(centroids_est(pli+1:end,3)<zbound(2));
...
%             pl=[pl;[ones(size(index_xyz,1),1)*pli, index_xyz]];
%             else
%             end
%         end
%         pl_tmp1=pl;
...
% n_ep=zeros(size(idest,1),1);
% for idi=1:size(idest,1)
...
%         index_id1=find(idest==id1(pairi,1));
%         index_id2=find(idest==id2(pairi,1));
...
% %     pl_tmp3(pair_vel_flowzone_pairmark(:,1)==0,:)=[];
%     pair_vel_flowzone_pairmark_record{i,1}=pl_tmp2;
...
%     end
%     %%
%     velo_fluc_coore_ep=zeros(size(idest,1),2);
%     for idi=1:size(idest,1)
...
% % plot(Gprecluster,'XData',id_centroids_radius_vxyz(1:idmax_residue,2),'YData', ...
% %                          id_centroids_radius_vxyz(1:idmax_residue,3),'ZData', ...
% %                          id_centroids_radius_vxyz(1:idmax_residue,4));
% % axis equal;
% % ylim([-0.1 0.1]);
% 
...
% idx = binsize(bin) > 2;%obtain max clusters
...
% for clusi=1:n
%     if size(bincell{1,clusi}',1)>1 && size(bincell{1,clusi}',1)<10000
...
% % cluster_x(clusj,1)=x(idest==atomid_clusterid(clusj,1));
% % cluster_y(clusj,1)=y(idest==atomid_clusterid(clusj,1));
% % cluster_z(clusj,1)=z(idest==atomid_clusterid(clusj,1));
% % end
% 
% % figure(1)
...
%    clear cluster_sizerecord
   ...
   clear velocity cntroids id radius id_centroids_radius_vxyz_record
