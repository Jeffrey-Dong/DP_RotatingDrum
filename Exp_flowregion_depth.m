...
% filenum=[152,157,161,153]';
filenum=[149,152:154,157,158,161,162,169:172]';
...
if ismember(filenum(fi),[158,166,149,172])
    omega_sample=60;
elseif ismember(filenum(fi),[0157,0165,0152,0171])
    omega_sample=90;
elseif ismember(filenum(fi),[0161,0164,0153,0170])
    omega_sample=120;
elseif ismember(filenum(fi),[0162,0163,0154,0169])
    omega_sample=150;
end

watercontent_sample=watercontentmatrix(strcmp(samplefile_name,exprec_raw));
strainrate_coef=2;
strainrate_thresh=omega_sample/180*pi*strainrate_coef;
% strainrate_thresh=1;
