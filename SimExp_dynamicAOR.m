...
% model_number=[31,33,35,37,39,41]';
% model_number=[33,53,54,59:69]';
...
model_number=[103,95,96,87,97]';% vary surface tension gamma
elseif model_var == "liquid_content"
model_number=[103,87,99:102]';% vary liquid content
elseif model_var == "all"
% model_number=[95,96,87,97]';
% model_number=[86:89,95:97]';
% model_number=[81,82]';
% model_number=[49:52,70]';
% model_number=[112,103,109:111]';
% model_number=[112:114,92]';%dry mu_r
% model_number=[129,121:123]';%dry mu_r
% model_number=[124,126]';%compare with exp w=120
model_number=[119,134]';%compare with exp w=90
% model_number=[130,131,119]';%compare with exp w=90
end
% model_number=[77];
%% results record
DAORt_den=[];
...
%% custom colormap
color=colormap(hot(6));

%%
for modeli=1:size(model_number,1)

datafile=(['e:\RoDrtest\RoDr_alphastart0-1\model' num2str(model_number(modeli,1))]);
load([datafile '\model' num2str(model_number(modeli,1)) '_up_surface_points.mat']);
...
omega=keypara(model_number(modeli,1),5)/360;%revolution/s
dp=keypara(model_number(modeli,1),7);
V_p=4/3*pi*(dp/2)^3;
Rp=dp/2;
...
%         DAOR(modeli,1)=max(fitangle_a);
          DAOR(modeli,1)=max(fitangle_a(find(xfit_range>-0.01,1):end)); 
...
%             'Color',color(modeli*3,:));
        ...
        fig1=plot(xfit_range/dp,surf_mid/dp, ...
            '-', ...
            'LineWidth',1, ...
            'Color',color(modeli*3,:));
        fill(xfit_range2/dp, inBetween/dp, ...
...
%         
    ...
    %% ===========save results=============
%    cd(datafile);
%    datafilename=['model' num2str(model_number(modeli,1)) '_DAOR_gof.mat'];
...
legend(['dry; DAOR=' num2str(DAOR(1))], ...
    ['\gamma=0.0073N/m; DAOR=' num2str(DAOR(2))], ...
    ['\gamma=0.0365N/m; DAOR=' num2str(DAOR(3))], ...
    ['\gamma=0.073N/m; DAOR=' num2str(DAOR(4))], ...
    ['\gamma=0.146N/m; DAOR=' num2str(DAOR(5))], ...
    'Location','south', ...
    'fontsize',12);
elseif model_var=="liquid_content"
legend(['dry; DAOR=' num2str(DAOR(1))], ...
...
plot(xfit_range/dp+5,surf_mid/dp-2, ...
            '--', ...
            'Color',color(1,:), ...
            'LineWidth',1);
fill(xfit_range2_exp/dp+5, inBetween_exp/dp-2, ...
    color(1,:), ...
    'FaceAlpha',0.3,'LineStyle','none');
%% ===========================================
% % extract calibration and Container center info
...
plot(xfit_range/dp+5,surf_mid/dp-1, ...
            '--', ...
            'Color',color(2,:), ...
            'LineWidth',1);
fill(xfit_range2_exp/dp+5, inBetween_exp/dp-1, ...
    color(2,:), ...
    'FaceAlpha',0.3,'LineStyle','none');
