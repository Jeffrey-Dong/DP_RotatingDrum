...
clearvars -except modeli modelnum num txt raw keypara density angi angle_threshold angle_threshold_array pldist_threshold_coe_dry clustermethod dotvel_coe;
...
%% pair information: create pairlist based on pair force
    ...
    %     cd([Foldersdir_pairwise]);
        ...
        pl_tmp1(:,4)=id_centroids_radius_vxyz(pl_tmp1(:,1),5)+id_centroids_radius_vxyz(pl_tmp1(:,2),5);% r1+r2
        pl_tmp1_vxindex=id_centroids_radius_vxyz(pl_tmp1(:,1),6);
        pl_tmp1_vzindex=id_centroids_radius_vxyz(pl_tmp1(:,1),8);
        pl_tmp1(pl_tmp1_vxindex>0|pl_tmp1_vzindex>0,:)=[];
...
%% save to mat file
   ...
   atominfo_datafilename=['E:\RoDrtest\RoDr_alphastart0-1\model' num2str(modelnum(modeli,1)) '\model' num2str(modelnum(modeli,1)) '_clustermethod_' char(clustermethod) '_Angle' num2str(angle_threshold/pi*180) '_vcorr_conn_pldist_saminter5.mat'];
   end
...
%% plot probability distribution of granular temperature each particle and corresponding particle position
% figure(1)
% histogram(sqrt(sum(n_ep.^2,2)),'Normalization','Probability');
% set(gca, 'XScale', 'log')
% set(gca, 'YScale', 'log')
% figure(2);
% scatter3(id_centroids_radius_vxyz(:,2),id_centroids_radius_vxyz(:,3),id_centroids_radius_vxyz(:,4),[],sqrt(sum(n_ep.^2,2)));