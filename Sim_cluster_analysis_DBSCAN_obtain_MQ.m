...
%     tic
    if modelnum(modeli,1)<118
        ...
        [id,~,x,y,z,~,~,~,vx,vy,vz,fx,fy,fz,~,~,~,radius,VliqonVatom]=textread( ...
            txt_read_atominfo(sample_step(i)).name,'%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f','headerlines',9);
        vel=sqrt(vx.^2+vy.^2+vz.^2);
    end
    id_centroids_radius_vxyz=[id,x,y,z,radius,vx,vy,vz,fx,fy,fz];
    ...
    radius_est=id_centroids_radius_vxyz(:,5);%each step
...
%     toc
    ...
    radius_est_strrfilted=radius_est(index_abovestrrbound,:);
    centroids_est_filtered_record{i,1}=centroids_est_strrfilted;
    id_est_filtered_record{i,1}=id_est_strrfilted;
    radius_est_filtered_record{i,1}=radius_est_strrfilted;
...
%% save to mat file
   ...
   atominfo_datafilename=['E:\RoDrtest\RoDr_alphastart0-1\model' num2str(modelnum(modeli,1)) '\model' num2str(modelnum(modeli,1)) '_MQ.mat'];

   save(atominfo_datafilename,'centroids_est_filtered_record','-v7.3');
       clear centroids_est_filtered_record
   save(atominfo_datafilename,'id_est_filtered_record','-append');
       clear id_est_filtered_record
   save(atominfo_datafilename,'radius_est_filtered_record','-append');
       clear radius_est_filtered_record
